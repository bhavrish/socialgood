...
for num in range(0, range_length):
    ...
    zip_and_counts.append([zip_list[num], zip_list.count(zip_list[num]), time_list[num]])
    #if (zip_list[num] == 0):
    #    print(num)
    #else:
...
with open('zips_and_counts.csv', mode='w', newline='') as employee_file:
    ...
    for num in range(0, range_length):
        ...
        if (zip_and_counts[num][0] == 0):
            continue
        else:
            writer.writerow([zip_and_counts[num][0], zip_and_counts[num][1], time_list[num], (int(zip_and_counts[num][1] / 1000))+1])
